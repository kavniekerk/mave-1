        ...
        with PdfPages('report_%s.pdf'%(fname,)) as pdf:
           ...
           #             rasterized=True)
            ...
            if np.greater(max_Month,self.X['Month'][0]-1) and \
               np.greater(max_Month,self.X['Month'][-1]):
                if np.less(self.X['Month'][0],self.X['Month'][-1]+2):
                    ...
                    Months = list(range(int(self.X['Month'][0]), 13))+\
                             list(range(1,int(self.X['Month'][-1]+1)))
            else:
                Months = list(range(int(self.X['Month'][0]),\
            ...
            labels = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug',\
                      'Sep','Oct','Nov','Dec']
            plt.xticks(list(range(1,13)),labels)
            plt.xlim(0.5,12.5)
            pdf.savefig(fig4)
            plt.close()
            pdb.set_trace()
           ...
           # pt_Hour_p[row_pos,col_pos] = comparison
            ...
            d = pdf.infodict()
            d['Title'] = "Report"
            d['Author'] = "report automatically generated by Mave"
